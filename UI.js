...
function UI() {
    ...
    const [playBombSound ] = useSound(bomb);
   ...
   // Modify Rocket Speed
    useEffect(()=>{
      const interval = setInterval(() => {
      const newSpeed = Math.floor(Math.random()*100 + 1);
      setRocketSpeed(newSpeed);
      }, 100);
  
      return () => {
        clearInterval(interval);
      };
    },[rocketX, rocketSpeed, multiplier])
    ...
    <br />
    <br />
   <button className='start' onClick={()=>{window.location.reload()}}> Start </button>
</>
    )
}
